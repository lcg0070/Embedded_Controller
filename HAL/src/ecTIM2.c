...
void TIM_init(TIM_TypeDef* TIMx, uint8_t sec_flag, uint32_t sec){
	...
	// for (int i = 0; i < sizeof(configs) / sizeof(TIM_Config); i++) {
	// 	if (configs[i].TIMx == TIMx) {
	// 		*(configs[i].reg) |= configs[i].bit;
	// 		break;
	// 	}
	// }
    ...
    // 2. Set CNT period
	if(sec_flag){
		TIM_period_ms(TIMx, sec);
	}else {
		TIM_period_us(TIMx, sec);
	}
...
void TIM_period(TIM_TypeDef* TIMx, uint8_t sec_flag, uint32_t sec){
	if(sec_flag) TIM_period_ms(TIMx, sec);
	else TIM_period_us(TIMx, sec);
}
