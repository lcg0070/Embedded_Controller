...
void PWM_init(PinName_t pinName, uint8_t secflag, uint32_t sec) {
	...
	GPIO_otype(pinName, OUTPUT_PUSH_PULL);
	GPIO_pupd(pinName, PULL_UP);
	GPIO_ospeed(pinName, HIGH_SPEED);

	// 2. Configure GPIO AFR by Pin num.
	//  AFR[0] for pin: 0~7,     AFR[1] for pin 8~15
	//  AFR=1 for TIM1,TIM2	AFR=2 for TIM3 etc
	int AFR_idx = pin>>3;
	port->AFR[AFR_idx] &= ~((0b1111)<< 4*pin);		// clear
	if(TIMx == TIM1 || TIMx == TIM2)							port->AFR[AFR_idx] |= (0b0001<< 4*(pin%8));
	else if(TIMx == TIM3 || TIMx == TIM4 || TIMx == TIM5)		port->AFR[AFR_idx] |= (0b0010<< 4*(pin%8));
	else														port->AFR[AFR_idx] |= (0b0011<< 4*(pin%8));

// 3. Initialize Timer
	TIM_init(TIMx, secflag, sec);	// with default msec=1msec value.
...
// 4. Configure Timer Output mode as PWM
	...
	if(chN == 1){
		...
		TIMx->CCMR2 |= TIM_CCMR2_OC4M_1 | TIM_CCMR2_OC4M_2; // OC1M = 110 for PWM Mode 1 output on ch2
		TIMx->CCMR2	|= TIM_CCMR2_OC4PE;                     // Output 1 preload enable (make CCR2 value changable)
		TIMx->CCR4  = ccVal; 								// Output Compare Register for channel 2 (default duty ratio = 50%)
		TIMx->CCER &= ~TIM_CCER_CC4P;                       // select output polarity: active high
		TIMx->CCER  |= TIM_CCER_CC4E;						// Enable output for ch2
	}
...
void PWM_period(PinName_t pinName, uint8_t sec_flag, uint32_t sec){
	if(sec_flag) PWM_period_ms(pinName,  sec);	// allowable range for msec:  1~2,000
	else PWM_period_us(pinName, sec);			// allowable range for usec:  1~1,000
}
