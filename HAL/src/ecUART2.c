...
void USART_setting(USART_TypeDef* USARTx, PinName_t pin_GPIO_TX, PinName_t pin_GPIO_RX, uint32_t baud){
	...
	GPIO_otype(pin_GPIO_TX, OUTPUT_PUSH_PULL);
	GPIO_pupd(pin_GPIO_TX, NO_PULLUP_PULLDOWN);
	GPIO_ospeed(pin_GPIO_TX, HIGH_SPEED);
	
	GPIO_init(pin_GPIO_RX, AF);
	GPIO_otype(pin_GPIO_RX, OUTPUT_PUSH_PULL);
	GPIO_pupd(pin_GPIO_RX, NO_PULLUP_PULLDOWN);
...
// Configure Baud-rate 
	...
	// Method 1
	float USARTDIV = (float)(fCK/16.)/(float)baud;
	uint32_t mantissa = (uint32_t)USARTDIV;
	uint32_t fraction = round((USARTDIV - mantissa)*16);
	USARTx->BRR |= mantissa<<4 | fraction;

	// Method 2
	// float USARTDIV_div16 = (float)(fCK)/(float)baud;
	// uint32_t USART_BRR = (uint32_t)USARTDIV_div16;
	// USARTx->BRR |= USART_BRR<<4 - USART_BRR&0b1111<<4 + USART_BRR&0b1111;
