...
// 	TIM_UI_init(TIMx, M_SEC,1);        					// TIMx Interrupt initialize
//
...
//
// 	TIMx->CCER &= ~(0b1 << 4*(ICn - 1));			// CCn(ICn) Capture Enable
//
// // 6.	Enable Interrupt of CC(CCyIE), Update (UIE)
// 	TIMx->DIER |= (0b1<<ICn);						// Capture/Compare Interrupt Enable	for ICn
